...
tape('should fail when runCall fails', async (t) => {
  ...
  await suite.p.runBlock({ block, root: suite.vm.stateManager.trie.root, skipBlockValidation: true })
    .then(() => t.fail('should have returned error'))
    .catch((e) => t.equal(e.message, 'test'))
...
tape('should run valid block', async (t) => {
  ...
  let res = await suite.p.runBlock({ block, root: suite.vm.stateManager.trie.root, skipBlockValidation: true })
